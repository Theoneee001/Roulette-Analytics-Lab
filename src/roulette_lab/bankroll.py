...
class BankrollSimulation:
    """Immutable paths from a bankroll simulation, including initial capital."""
    ...
    def __post_init__(self) -> None:
        ...
        values = np.array(self.paths, dtype=float, copy=True)
        expected_shape = (self.config.paths, self.config.spins + 1)
        if values.shape != expected_shape:
            raise ValueError(f"paths must have shape {expected_shape}.")
        if not np.all(np.isfinite(values)) or np.any(values < 0.0):
            raise ValueError("paths must contain finite non-negative bankrolls.")
        if not np.all(values[:, 0] == self.config.initial_bankroll):
            raise ValueError("paths must include initial_bankroll in column zero.")
...
@dataclass(frozen=True, slots=True)
class RiskSummary:
    """Terminal-bankroll and running-peak drawdown metrics for all paths."""
...
def simulate_bankroll(
    ...
    for spin in range(config.spins):
        ...
        updated[settling & outcomes_win] += imprisoned_stake[settling & outcomes_win]
        imprisoned[settled] = False
        imprisoned_stake[settled] = 0.0
        requested_stakes[settled] = config.base_stake
        ...
        _advance_requested_stakes(
            requested_stakes, stakes, betting, wins, config.strategy, config.base_stake
        )
        paths[:, spin + 1] = updated

    return BankrollSimulation(config=config, paths=paths, payout_label=payout_label)
...
def summarize_bankroll(simulation: BankrollSimulation) -> RiskSummary:
    ...
    return RiskSummary(
        ...
        probability_of_ruin=float(np.mean(np.any(paths <= 0.0, axis=1))),
